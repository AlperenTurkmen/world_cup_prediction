...
-- =============================================================================
-- Migration 6: Knockout scorelines — records + scoring (manual + upload).
-- =============================================================================
-- Adds per-entry knockout scorelines and actual knockout results, folds a
-- knockout-scoreline dimension into the leaderboard's knockout_points, extends
-- create_entry / apply_master_results with a knockout payload, and adds
-- entry_drafts.ko_scores. Re-runnable. The function bodies below are copied
-- verbatim from db/schema.sql (the canonical full schema).
...
-- --- apply_master_results (gains p_knockout) ---------------------------------
...
-- --- Updated scoring: compute_leaderboard + leaderboard view + league board --
...
-- =============================================================================
-- Migration: Allow players to rename themselves up to 3 times
-- Run this in your Supabase SQL Editor.
-- =============================================================================
alter table entries
...
-- =============================================================================
-- Migration: global start-game floor + per-prediction validity + played count
-- Run this in your Supabase SQL Editor (idempotent / re-runnable).
--   * app_settings.global_start_match_id — a tournament-wide "everyone starts
--     from game N" floor for the PUBLIC board (set via /api/admin/global-start).
--   * compute_leaderboard / leaderboard / league_leaderboard now return
--     played_count (group + knockout scorelines that cleared the fairness gate).
--   * The global leaderboard view reads the floor from app_settings.
-- Per-prediction validity reuses the existing predictions.is_score_eligible
-- column (toggled by /api/admin/eligibility) — no DDL needed for that part.
-- =============================================================================
...
-- =============================================================================
-- Migration 8: Knockout per-round tours + foresight bonus
-- =============================================================================
-- Run this in your Supabase SQL Editor (idempotent / re-runnable). Adds the
-- second-tour knockout predictions and reshapes knockout scoring:
--   * round_tour_predictions — the per-round "second tour": after the group
--     stage each knockout round opens a fresh window where users predict the
--     REAL matchups (matches 73–102, 104), editable until the round's first
--     kickoff. Only the scoreline is stored; the teams come from
--     actual_knockout_matches by match_no.
--   * Scoring change in compute_leaderboard():
--       - The pre-tournament bracket's knockout scorelines (knockout_predictions,
--         the old "dimension E") NO LONGER score 8 each. They now grant a
--         FORESIGHT BONUS — that round's advancement weight (R32 +1, R16 +2,
--         QF +4, SF +6, FINAL +8) — only when the bracket nailed BOTH the exact
--         matchup AND the exact scoreline of a real game, foreseen blind.
--       - NEW dimension F: per-round tour scorelines, scored A-style (flat max 8
--         per game) vs the actual knockout result, counted only when the pick was
--         last edited before its round's first kickoff.
--       - played_count's knockout half now follows the TOUR predictions.
--   * The function signature, the leaderboard view, and league_leaderboard() are
--     unchanged in shape — only the body changes — so all callers keep working.
-- The function/view/table bodies below are copied verbatim from db/schema.sql
-- (the canonical full schema).
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- Migration 9: Third-place playoff (match 103) becomes a tour game + its row.
--   - actual_knockout_matches gets a row for 103 (kickoff from the real
--     schedule); the sync now upserts the matchup once the semi-final losers
--     are known, and its scoreline once the game finishes.
--   - compute_leaderboard: ko_match_round now maps 103 to its own one-game
--     round, THIRD, so a tour pick for it locks (and is deadline-gated) on its
--     OWN kickoff. THIRD has no round_weights row, so advancement + foresight
--     scoring are untouched. Dimension F picks for 103 score flat max 8 like
--     every other tour game.
-- =============================================================================

insert into actual_knockout_matches (match_no, kickoff_at)
values (103, '2026-07-18 21:00:00+00')
